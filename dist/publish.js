...
var cwd = process.cwd();
var sync_files = ["package.json", "commit.json"];
...
function writeVersion(version) {
  ...
  if (!import_fs.default.existsSync(gh_scripts)) {
    import_fs.default.mkdirSync(gh_scripts);
  }
  ...
  import_fs.default.writeFileSync(gh_package_path, JSON.stringify(package_json, null, 2));
}
function get_build_dir() {
  let cwd2 = process.cwd();
  let build_path = `${cwd2}/build`;
  let build_dirs = import_fs.default.readdirSync(build_path);
...
async function write_last_commit() {
  try {
    ...
    if (results.status == 200 && results.data.length > 0) {
      ...
      import_fs.default.writeFileSync(commit_path, JSON.stringify(commit_json, null, 2));
    }
  } catch (e) {
    console.error(e);
  }
